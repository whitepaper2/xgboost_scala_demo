...
//    data.map(row => fm_get_p(row.features, W))
//  }
//
...
//    */
//    val y_true_rdd = data.map(lp => if (lp.label == 1) {
//      1
//    } else {
//      0
//    })
...
//
//    for (i <- 1 to iter_sgd) {
//      var random_idx_list = shuffle(0 until N - 1)
//      for (j <- 0 until N - 1) {
...
//
//    var W = BDM.rand[Double](nrFeat, factorLength)
//    W :*= 1 / sqrt(sum(W :* W))
//
//    if (verbose) {
